<template>
  <div class="wrap">
    <!-- <vbt-table border stripe row-key="id" size="mini" isBigData isTreeTable show-summary highlight-hover-row
      max-height="600" :data="tableData">
      <vbt-table-column type="index" width="100">
      </vbt-table-column>
      <vbt-table-column v-for="item in columns" show-overflow-tooltip :key="item.value" :prop="item.value"
        :label="item.label" :width="item.width" :fixed="item.value === 'id'">
        <template slot-scope="scope">
          <el-input v-if="item.value === 'name'" size="mini" v-model="scope.row[item.value]"
...
import vtt from './btt/table'
import vttc from './btt/table-column'
